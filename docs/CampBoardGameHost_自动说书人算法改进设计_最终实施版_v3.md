...
> 修订重点：正交结果模型、候选家族概率、版本化规则知识、幂等事件与可验证回放
...
- 官方规则要求或禁止的内容进入合法性验证器；
- 官方允许、但本产品不希望默认采用的裁量，保留为合法候选，再由产品策略禁用、降级或降权；
- 温和、平衡、激进只影响风格偏好，不能改变官方合法性；
- 每条规则事实必须保存来源、规则版本和最后核验时间；
- 未核验角色或相互作用不得显示为“规则已确认”，自动模式应降级为人工确认。

例如，“健康图书管理员面对唯一实际隐士时默认显示隐士”是《暗流涌动》的产品策略，不应被误写成对所有剧本都成立的官方强制规则。
...
当前逻辑对非调查员的酒鬼显示角色整体降级，导致调查员在大量配置中成为事实上的唯一最高等级候选。
...
```kotlin
val scoreDelta = candidate.totalScore - bestScore
val scoreWeight = exp(scoreDelta / temperature)

val withinFamilyWeight =
    scoreWeight *
    historyMultiplier *
    diversityMultiplier *
...
val finalProbability =
    familyMass * withinFamilyWeight / familyWeightSum
```
...
- 分数越接近最佳值，权重越高；
- 低于容差的候选不会进入池；
- 新增同一家族的候选不能意外改变该家族的总概率；
- 不允许低质量候选仅因随机而被选中。

所有乘数必须定义上下限，并明确某个风险因素究竟进入 `totalScore` 还是进入概率乘数，避免同一因素被重复计算。
...
禁止使用对象默认 `hashCode()`、`toString()` 或依赖字段拼接顺序的哈希。
...
candidateId = stableHash(
    ...
    abilityRole,
    shownRole,
    sortedCandidateSeats,
    numericValue,
    registrationDetails,
)
```

实际实现必须使用规范化序列化：固定字段顺序、集合排序、明确空值、长度前缀和固定字符编码。随机数生成器、浮点/定点权重算法和候选 ID 算法都要有版本号及跨版本 golden vectors。优先使用定点整数权重，避免浮点边界差异改变选择结果。
...
│   ├── dynamic/
│   │   ├── DynamicCandidateGenerator.kt
│   │   ├── MalfunctionPolicy.kt
│   │   ├── RegistrationPolicy.kt
│   │   └── ConsequenceEvaluator.kt
│   ├── selection/
│   │   ├── WeightedStableSelector.kt
│   │   ├── CandidatePoolBuilder.kt
│   │   └── DecisionSeedFactory.kt
│   └── pressure/
│       ├── PlayerPressureCalculator.kt
│       ├── MisinformationLedger.kt
│       └── RegistrationLedger.kt
├── history/
│   ├── StorytellerEventStore.kt
│   ├── DecisionHistoryRepository.kt
│   └── HistoricalClueSignature.kt
├── simulation/
│   ├── StorytellerSimulationRunner.kt
│   ├── DistributionReport.kt
│   └── RegressionScenarioCatalog.kt
└── session/
    ├── ClocktowerGameSession.kt
    └── ClocktowerRecommendationCoordinator.kt
```
...
- 持久化 `gameSeed`；
- `RulesetRef`、内容哈希和规则核验状态；
- 从 MainActivity 抽出最小会话状态边界，不进行全面 UI 重写。

### PR 2：自然真实热修复

- 健康图书管理员面对唯一隐士时，100%显示隐士；
