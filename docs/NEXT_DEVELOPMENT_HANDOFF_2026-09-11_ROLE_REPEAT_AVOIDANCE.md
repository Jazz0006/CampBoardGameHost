...
> Status: **COMPLETE / T4 VALIDATED — draft PR #119 awaiting explicit merge authorization**  
...
4. this handoff only if PR #119 is still open and its closeout evidence is needed;
...
The implemented behavior is **not** deterministic rotation and **not** a change to which roles belong in the setup. It preserves randomness while improving short-run player experience.
...
The setup always remains legal and available. If a preferred zero-repeat permutation is impossible, accept the minimum-cost legal assignment. Rotation never becomes a setup legality rule.
...
## 10. R6 final acceptance — complete

Explicit T4 checkpoint:
...
R2 main-thread boundary #2136 / 34556109649 PASS
```

This T4 checkpoint subsumes the affected persistence/history/production-wiring regression surface and is the authoritative final acceptance evidence for ROLE-ROTATION-1.

R1–R6 are complete. No further production changes are required for this initiative unless review or real-device testing finds a new defect.

PR #119 remains draft. Do not merge it without explicit user authorization.
...
After PR #119 is explicitly authorized and merged, return to `docs/CURRENT_DEVELOPMENT_ROADMAP.md`. The next queued program is currently EPI-MQ / Productive Uncertainty unless reprioritized by the user. Re-audit its older handoff against then-live `main` before implementation.

## 14. UI-NAV-1 inherited status

UI-NAV-1 is closed for feature development. PR #118 received real-device acceptance after the identity-controller width and night-flow bottom-navigation follow-up fixes. Do not reopen that campaign while reviewing ROLE-ROTATION-1 unless an actual regression is reproduced.
