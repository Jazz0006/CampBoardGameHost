...
- **Current C4 checkpoint:** CR-A / CR-B / CR-C remain COMPLETE. C4 / SDE-3D2 T0-A typed truth/credibility owner, T0-B Red-Herring setup-precommit adapter, exact truth-danger source projection and the Evin E2 semantic regression are COMPLETE. Validation-only Draft PR #158 exact head `f181780bac2ff7759a880821ab664d7d23952131` is GREEN with CI #3468 + R2 #3221, including Android FULL, ASP contracts, Real Clingo and final CI gate. The exact accepted 10 C4 code/test blobs were then persisted as one fast-forward formal-branch commit `d39b336145b19ba29690a5b2dd14ed38273a6cc5`; the validation-only `AGENTS.md` synchronization was explicitly excluded. Next executable work is V1 invariance plus explicit DecisionTrace typed truth/credibility schema evolution and replay compatibility. V1 stays immutable and no placeholder V2 is authorized.

Upstream development branch: `sde-3c-decision-trace-shadow-replay` (local continuation branch above).

Latest formal integration code checkpoint: `d39b336145b19ba29690a5b2dd14ed38273a6cc5` — a single fast-forward commit containing exactly the 10 C4 code/test files accepted on validation head `f181780bac2ff7759a880821ab664d7d23952131`. Acceptance evidence belongs to validation-only Draft PR #158: CI #3468 SUCCESS and R2 #3221 SUCCESS. Earlier accepted checkpoints remain CR-A/B/C validation `4245ddb8` (CI #3459 / R2 #3212), SDE-3D1 `f562887c` (CI #3446 / R2 #3201) and SDE-3C5 `db7d5575`. Documentation-only synchronization commits may advance the formal branch; always query live refs.
