...
- Evin primary-source provenance / extraction boundary: [`SDE_2D5F_EVIN_FIRST_PLAYTHROUGH_SOURCE_AUDIT_2026-09-22.md`](SDE_2D5F_EVIN_FIRST_PLAYTHROUGH_SOURCE_AUDIT_2026-09-22.md)
...
The source catalog now tracks one admitted GOLD case plus four remaining GOLD candidates:
...
Ben's experienced/trusted status is independently supported by TPI. `A Stud In Scarlet` is now primary-verified and admitted GOLD; `Human Remains Of The Day` and `Live and Imp-Person` still require primary-video verification. `A Fond Farewell` now has primary-verified material state and explicit rationale, but executable reconstruction is blocked by missing production Traveller semantics. Evin remains the next strong primary-source target and independently qualifies through his TPI co-founder role, integral early involvement, and founding of the Newcastle Clocktower group. His source provenance is now triangulated; the remaining blocker is direct extraction of the actual seating/setup/Night-1 state, not another metadata search.

### D5F-B4C — reconstruct expert Night 1 decisions — IN PROGRESS

Executable canonical reconstructions now exist for `A Stud In Scarlet`, `Human Remains Of The Day`, and `Live and Imp-Person`. All three have committed-prefix consequence calibration through the dedicated `:app:sde2D5FExpertObservedCalibration` T3 task/workflow and are published together as one report artifact. `A Stud In Scarlet` is now `PRIMARY_VERIFIED` and admitted GOLD because its primary-source gate is satisfied; the other two remain `PRIMARY_VERIFICATION_PENDING`.
