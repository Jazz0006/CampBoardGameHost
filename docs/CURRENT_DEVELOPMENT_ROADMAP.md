...
MS-S3    optional template repository            NEXT
```
...
`SetupCandidate` is deliberately a **pre-seat actual-role multiset**, not a committed setup:

```text
SetupCandidate
├─ script: ScriptId
├─ actualRoles: canonical/snapshotted role multiset
├─ playerCount: derived
└─ provenance: SetupProvenance
```

Deliberately absent from generic candidate core:

- seat assignment;
- shown identities / Drunk shown-role options;
- persistence schema;
- diversity history/scoring;
- TB `source`, `complexity`, `styleTags`, runtime selection policy and rotation weights.
...
`ClocktowerSetupProvider` rejects cross-script requests, mismatched candidate player counts, cross-script candidates and cross-provider attribution. Empty candidate lists remain valid for later optional-source/fallback composition.
...
MS-S3 adds the smallest optional template repository boundary keyed by script + player count.

Target responsibilities:

```text
TemplateRepository
request(script, playerCount)
...
2. audit `TroubleBrewingSetupPresetDataset.pools` and TB dataset validation ownership;
3. freeze the generic template-record/repository boundary without importing TB scoring/style/shown-role policy into core;
4. preserve `SetupCandidateSource` as the outer source contract;
5. prove empty/no-template lookup is a normal supported result, not an error;
6. prove script/player-count mismatch cannot leak candidates across buckets;
7. keep repository persistence-independent and Android-free;
8. stop before deterministic generated setup implementation (MS-S4) and before TB production adaptation (MS-S7).

Do not make the generic repository depend on the frozen TB asset format itself. TB adaptation should happen through an adapter/provider in its later slice.
...
MS-S2 is accepted at `d4001863f134ebbe7d26819f40ac34c7d1de200c` with CI #1225 and R2 #1142.
...
MS-S3 should be small pure Kotlin work and should not require App/Host edits.
...
3. read MS-S1/MS-S1R/MS-S2 checkpoints as relevant;
...
6. treat `d4001863f134ebbe7d26819f40ac34c7d1de200c` as the accepted MS-S2 code/test checkpoint unless later production commits deliberately supersede it;
7. next production slice is MS-S3;
8. stop MS-S3 before generated-source implementation (MS-S4);
...
| Area | Status |
|---|---|
| MS-SETUP generic multi-script setup architecture | CURRENT — MS-S3 NEXT |
| MS-S1R setup persistence authority migration | COMPLETE / ACCEPTED |
