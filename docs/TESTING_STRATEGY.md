...
`FirstNightBundleBeginnerCorpusExperiment`, `Sde2D4ScaleBenchmarkTest`, and `Sde2D5CalibrationExperiment` are also excluded from the underlying `:app:testDebugUnitTest` / `:app:testFull` task because they are explicit evidence-generation harnesses rather than bounded regression contracts. They remain directly runnable through `:app:fnBundle3Calibration`, `:app:sde2D4ScaleBenchmark`, and `:app:sde2D5Calibration`.
...
| `Sde2D5CalibrationExperiment` | 5m53s observed when 8 cross-regime topology points were temporarily placed in FAST | deterministic 5–15 STANDARD/BARON calibration evidence generation | explicit T3 evidence harness, dedicated task only | D5 calibration / policy evidence |
